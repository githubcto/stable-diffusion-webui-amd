...
case "$gpu_info" in
    ...
    *"Navi 3"*) [[ -z "${TORCH_COMMAND}" ]] && \
    ...
    ;;
    *"Strix Halo"*) [[ -z "${TORCH_COMMAND}" ]] && \
         export TORCH_COMMAND="pip install torch torchvision --index-url https://download.pytorch.org/whl/rocm7.1"
    ;;
    *"Strix Point"*) [[ -z "${TORCH_COMMAND}" ]] && \
         export TORCH_COMMAND="pip install --pre torch torchvision --index-url https://download.pytorch.org/whl/nightly/rocm7.2"
    ;;
    *"Kraken Point"*) [[ -z "${TORCH_COMMAND}" ]] && \
         export TORCH_COMMAND="pip install --pre torch torchvision --index-url https://download.pytorch.org/whl/nightly/rocm7.2"
    ;;
    *"Navi 4"*) [[ -z "${TORCH_COMMAND}" ]] && \
         export TORCH_COMMAND="pip install --pre torch torchvision --index-url https://download.pytorch.org/whl/nightly/rocm7.2"
    ;;
